              ...
              .then(url => {
                const uploadTask = () => {
                    ...
                    if(moment(startDate).format('L') !== moment(new Date()).format('L')) {
                        ...
                        addNote({documentId: user.displayName, noteId: title, author: user.username, text: note, img: url, status: status, hour: null, markup: marker, userId: user.uid, date: date})
