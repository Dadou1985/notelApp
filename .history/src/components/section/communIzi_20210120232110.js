...
export default function CommunIzi({firebase, user}) {
    ...
    const handleSubmit = (event) =>{
        ...
        if(expanded === ''){
          ...
          return firebase.addMessageIziChat({roomName: expanded, author: user.username, text: note, mood: mood, hour: time, userId: user.uid, date: date})
        }
    }
    ...
    const handleRoomnameSubmit = (event) => {
      event.preventDefault()
      setRoom('')
      firebase.addChatRoom({roomName: room, userId: user.username})
      handleClose()
    }
    ...
    return (
        <div style={{width: "90%"}}>
            ...
            <div id="box" className="communizi_notebox">
                {info.map((flow) => (
                      <Accordion square expanded={expanded === flow.id} onChange={handleChangePanel(flow.id)}>
                      <AccordionSummary aria-controls={flow.id} id={flow.id}>
                        <Typography>{flow.id}</Typography>
                      </AccordionSummary>
                      <AccordionDetails>
                        <Typography style={{width: "100%", color: "white"}}>
                          {!!firebase &&
            ...
            centered>
                ...
                <Modal.Body>
                    <Input type="text" placeholder="nom de la conversation" value={room} style={{borderTop: "none", borderLeft: "none", borderRight: "none"}} maxLength="100" onChange={handleChangeRoomName} />
                </Modal.Body>
                <Modal.Footer style={{borderTop: "none"}}>
                    <Button variant="success" onClick={handleRoomnameSubmit}>Créer</Button>
                </Modal.Footer>
            </Modal>
            <Snackbar open={open} autoHideDuration={6000} onClose={handleCloseSnackbar} style={{position: "absolute", left: "5vw", bottom: "1vh"}}>
              <Alert onClose={handleClose} severity="danger">
                Vous avez oublié de sélectionner une discussion !
              </Alert>
            </Snackbar>
        </div>
    )
