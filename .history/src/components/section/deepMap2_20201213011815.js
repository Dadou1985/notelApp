    ...
    useEffect(() => {
     ...
     useEffect(() => {
        ...
        const signal = abortController.signal
            ...
            firebase.hotelRatingDataOnAir({region: region, departement: departement, hotelId: info.id, signal : signal}).onSnapshot(function(snapshot) {
                const snapInfo = []
              snapshot.forEach(function(doc) {          
                snapInfo.push({
                    id: doc.id,
                ...
                {info.map((hotel, key) =>(
                    ...
                    > 
                     <OverlayTrigger
                        placement="top"
                        overlay={
                        <Tooltip id="title" onMouseOver={()=>{setHotelRef(hotel.id)}} onMouseLeave={()=>{setHotelRef(null)}}>
                            ...
                            <Modal show={list}
                                ...
                                onClick={(event) => {
                                    ...
                                    firebase.addCommentOnHotel({
                                        ...
                                        team: firebase.firestore.FieldValue.arrayUnion(formValue.team),
                                        management: firebase.firestore.FieldValue.arrayUnion(formValue.management),
                                        customer: firebase.firestore.FieldValue.arrayUnion(formValue.customer),
                                        wage: firebase.firestore.FieldValue.arrayUnion(formValue.wage)})
