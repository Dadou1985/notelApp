...
export default function DeepMap2({user, firebase}) {
    ...
    const [hotelSelected, sethotelSelected] = useState(initialState)
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        ...
        if(commentData !== null){
            firebase.hotelRatingDataOnAir({region: region, departement: departement, hotelId: hotelSelected, signal : signal}).onSnapshot(function(snapshot) {
                const snapInfo = []
              snapshot.forEach(function(doc) {          
                snapInfo.push({
                    id: doc.id,
