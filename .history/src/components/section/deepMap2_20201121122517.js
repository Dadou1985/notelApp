...
import Map, {Marker, LinearInterpolator, FlyToInterpolator} from 'react-map-gl'
...
export default function DeepMap2({user, firebase}) {
    ...
    handleChangeViewport = (lat, lng, newZoom) => {
        const viewport = {
            ...viewport,
            longitude: lng,
            latitude: lat,
    ...
    const handleDepartement = (departement) => {
            ...
            <div style={{
                ...
                {DepartementDetails.map(details => (
                    <Dropdown.Item  onClick={()=>{
                        handleDepartement(details.nom)}}>{details.nom}</Dropdown.Item>
                        handleChangeViewport(details.coordinates[0], details.coordinates[1], details.view)
                    ))}
                </DropdownButton>
                </Form.Group>
                    </Form.Row>
