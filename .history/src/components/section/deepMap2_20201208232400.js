...
export default function DeepMap2({user, firebase}) {
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        ...
        if(selectedHotel !== []){
            firebase.hotelRatingDataOnAir({region: region, departement: departement, hotelId: selectedHotel.id, signal : signal}).onSnapshot(function(snapshot) {
                const snapRating = []
              snapshot.forEach(function(doc) {          
                snapInfo.push({
                    id: doc.id,
                ...
                console.log(snapRating)
                setCommentData(snapRating)
            });
     ...
     },[region, departement, selectedHotel])
                     ...
                     <OverlayTrigger
                        ...
                        <Tooltip id="title">
                            ...
                            <Box component="fieldset" mb={3} borderColor="transparent">
                                ...
                                <Rating
                                name="management"
                                //value={commentData !== null ? commentData.reduce((accumulator, currentValue) => accumulator + currentValue) : null}
                                precision={0.5}
