...
export default function DeepMap2({user, firebase}) {
    ...
    const [departement, setDepartement] = useState(["YVELINES", "Tous les départements"])
    ...
    {/*useEffect(() => {
        ...
        firebase.hotelDataOnAir({region: region, departement:  initialFilter: initialFilter, filter: filter, signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
