...
export default function DeepMap2({user, firebase}) {
    ...
    const handleDepartement = (departement, lat, lng, newZoom) => {
        ...
        flexFlow: "row"}}>
            ...
            <div style={{
                ...
                {DepartementDetails.map(details => (
                    <Dropdown.Item  onClick={()=>{handleDepartement(details.nom, details.coordinates[0], details.coordinates[1], details.zoom)}}>{details.nom}</Dropdown.Item>
                    ))}
                </DropdownButton>
                </Form.Group>
                    </Form.Row>
