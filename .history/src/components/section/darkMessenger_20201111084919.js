...
const DarkMessenger = () =>{
    ...
    const [title, setTitle] = useState("CommunIzi")
    ...
    return(
        <div style={{
          ...
          <div className="dark_messenger_container">
            <h5 className="font-weight-bolder dark_messenger_title">{title}</h5>
            {!!firebase && !!user &&
            <CommunIzi firebase={firebase} user={user} style={{display: "none"}} />}
        </div>
      </div>
    )
}
