...
export default function DeepMap2({user, firebase}) {
    ...
    const [arrondissement, setArrondissement] = useState("75001")
    ...
    const handleArrondissement = (arrondissement, lat, lng) => {
            ...
            {<div style={{
                ...
                {region === "PARIS" ? deptDetails[number].map(details => (
                    <Dropdown.Item  onClick={()=>{
                        console.log(details.view)
                        setZoom(details.view)
                        setInitialFilter("arrondissement")
                        setFilter(details.nom)
                        handleArrondissement(details.nom, details.coordinates[0], details.coordinates[1])}}>{details.nom}</Dropdown.Item>
