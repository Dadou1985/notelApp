...
const Dilema = () => {
  ...
  const handleChangePhotoUrl = (event) => {
    ...
    const handleSubmit = async(event) => {
        ...
        firebase.updateIziProfile({username: user.displayName, hotelName: formValue.hotelName, job: formValue.job, level: formValue.level, mood: formValue.mood}).then(handleClose)
        ...
        <Dialog showDialog={showDialog} hideShow={hideDialog} style={{display: "none"}}>
            <div style={{
                ...
                <img src={Fom} alt="Fom" style={{
                    position: "absolute",
                    filter: "invert() drop-shadow(-1px 1px 1px)",
                    width: "35%",
                    top: "35vh"}} />
            </div>
