...
const NoteBox = ({user, firebase}) => {
    ...
    const onSwitchChange = (e) => {
      let switchNote = [...switchState];

      if (e.checked)
          switchNote.push(e.value);
      else
          switchNote.splice(switchNote.indexOf(e.value), 1);

      setSwitchState(switchNote);
  }
                ...
                {messages.map((flow, index) => {
                  ...
                  <AccordionItem key={flow.id} onClick={() => setExpanded(index)} className="user_Message bg-light">
                      ...
                      borderTopRightRadius: "5px"
                        ...
                        <AccordionItemButton style={{
                         ...
                         <Avatar 
                          ...
                          <Switch
                              style={{zIndex: "50"}}
                              checked={switchState.indexOf(flow.markup) !== -1}
                    ...
                    <AccordionItemPanel style={{
                      ...
                      {flow.text}
                    <div><i style={{color: "gray", float: "right"}}> noté à {flow.hour}</i></div>
                    </AccordionItemPanel>
                  </AccordionItem> 
                  :
