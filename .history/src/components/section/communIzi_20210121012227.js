...
export default function CommunIzi({firebase, user}) {
    ...
    const handleChangeEvent = event => {
      setExpanded(event.currentTarget.eventKey)
    }
        ...
        <div style={{width: "90%"}}>
            ...
            <div className="communizi_notebox">
                {info.map((flow) => (
                  ...
                  <Accordion defaultActiveKey={flow.id} style={{marginBottom: "1vh"}}>
                    <Card style={{border: "none"}}>
                      <Accordion.Toggle as={Card.Header} eventKey={flow.id} style={{backgroundColor: "rgb(33, 35, 39)"}} onChange={handleChangeEvent}>
                        <div style={{diplay: "flex", flexFlow: "row", justifyContent: 'space-between'}}>
                        {flow.id}{flow.user}
                        </div>
                      </Accordion.Toggle>
                      <Accordion.Collapse eventKey={flow.id} style={{backgroundColor: 'lightgray'}}>
                        <Card.Body>
                          {!!firebase &&
