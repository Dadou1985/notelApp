...
const Annuaire = () =>{
    ...
    setFormValue("")
    let marker = Date.now()
    db.addContact({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, name: formValue.name, mobile: formValue.mobile, fix: formValue.fix, markup: marker}).then(handleClose)
    }
    ...
    useEffect(() => {
        ...
        db.contactOnAir({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
                                ...
                                {info.map(flow =>(
                                    ...
                                    key={flow.markup}>
                                        <div style={{padding: "2%", width: "50%"}}>
                                            ...
                                            <Button variant="outline-danger" size="sm" onClick={()=>db.deleteDocument({documentId: user.displayName, collection: "contact", document: flow.id})}>Supprimer</Button>
                                    </div>
                                ))}
                            </Tab>
