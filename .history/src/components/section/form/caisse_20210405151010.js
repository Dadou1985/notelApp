...
const Caisse = () =>{
    ...
    const handleChange = (event) =>{
      ...
      const handleSubmit = event => {
        ...
        handleReset()
        return db.addSafe({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, author: user.displayName, amount: caisse, date: time, markup: marker, shift: formValue.shift}).then(handleClose)
    }
    ...
    useEffect(() => {
        ...
        db.safeOnAir({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
                            ...
                            <Table striped bordered hover size="sm" className="text-center">
                                ...
                                <tbody>
                                    {info.map(flow =>(
                                        ...
                                        <td>{flow.shift}</td>
                                        <td>{flow.date}</td>
                                        <td className="bg-dark"><Button variant="outline-danger" size="sm" onClick={()=>db.deleteDocument({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, collection: "safe", document: flow.id})}>Supprimer</Button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </Table>
