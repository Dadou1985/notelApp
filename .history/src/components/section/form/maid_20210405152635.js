...
const Maid = () =>{
    ...
    const handleChange = (event) =>{
      ...
      const handleSubmit = event => {
        ...
        let marker = Date.now()
        return db.addMaid({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, author: user.displayName, fromRoom: formValue.fromRoom, client: formValue.client, markup: marker, date: time, toRoom: formValue.toRoom, reason: formValue.reason, details: formValue.details, state: formValue.state}).then(handleClose)
    }

    useEffect(() => {
        const abortController = new AbortController()
        const signal = abortController.signal
        db.toolOnAir({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, collection: "maid", signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
        ...
        <div>
            ...
            overlay={
              <Tooltip id="title">
                ...
                <img src={ChangeRoom} className="icon" alt="contact" onClick={handleShow} style={{width: "50%", marginLeft: "20%"}} />
            </OverlayTrigger>
                            ...
                            <Table striped bordered hover size="sm" className="text-center"  style={{overflowX: "auto",
                                ...
                                <tbody>
                                    {info.map(flow =>(
                                        ...
                                        <td>{flow.date}</td>
                                        <td>{flow.author}</td>
                                        <td className="bg-dark"><Button variant="outline-danger" size="sm" onClick={()=>db.deleteDocument({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, collection: "maid", document: flow.id})}>Supprimer</Button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </Table>
