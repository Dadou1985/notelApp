...
const Cab = () =>{
    ...
    const handleSubmit = event => {
        ...
        setFormValue("")
        let marker = Date.now()
        db.addCab({hotelId: userDB.hotelId, author: user.displayName, room: formValue.room, client: formValue.client, markup: marker, date: formValue.date, hour: formValue.hour, destination: formValue.destination, pax: formValue.passenger, model: formValue.model}).then(handleClose)
    }

    useEffect(() => {
        const abortController = new AbortController()
        const signal = abortController.signal
        firebase.toolOnAir({hotelId: userDB.hotelId, collection: "cab", signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
