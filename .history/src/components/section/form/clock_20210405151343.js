...
const Clock = () =>{
    ...
    const handleSubmit = event => {
        ...
        let marker = Date.now()
        return db.addClock({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, author: user.displayName, room: formValue.room, day: formValue.date, client: formValue.client, markup: marker, date: time, hour: formValue.hour}).then(handleClose)
    }
    ...
    useEffect(() => {
        ...
        db.toolOnAir({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, signal : signal, collection: "clock"}).onSnapshot(function(snapshot) {
                    const snapInfo = []
                        ...
                        <Table striped bordered hover size="sm" className="text-center">
                            ...
                            <tbody>
                                {info.map(flow =>(
                                    ...
                                    <td>{flow.date}</td>
                                    <td>{flow.author}</td>
                                    <td className="bg-dark"><Button variant="outline-danger" size="sm" onClick={()=>db.deleteDocument({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, collection: "clock", document: flow.id})}>Supprimer</Button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </Table>
