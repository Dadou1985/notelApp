...
const Repair = () =>{
    ...
    const handleChange = (event) =>{
      ...
      const handleSubmit = event => {
        ...
        let marker = Date.now()
        return db.addMaintenance({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, author: user.displayName, room: formValue.room, client: formValue.client, markup: marker, date: time, type: formValue.type, details: formValue.details}).then(handleClose)
    }
    ...
    useEffect(() => {
        ...
        db.toolOnAir({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, collection: "maintenance", signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
                            ...
                            <Table striped bordered hover size="sm" className="text-center">
                                ...
                                <tbody>
                                    {info.map(flow =>(
                                        ...
                                        <td>{flow.date}</td>
                                        <td>{flow.author}</td>
                                        <td className="bg-dark"><Button variant="outline-danger" size="sm" onClick={()=>db.deleteDocument({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, collection: "maintenance", document: flow.id})}>Supprimer</Button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </Table>
