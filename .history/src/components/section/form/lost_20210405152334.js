...
const Lost = () =>{
    ...
    const handleChange = (event) =>{
      ...
      const handleSubmit = event => {
        ...
        let marker = Date.now()
        return db.addLostFound({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, author: user.displayName, date: time, type: formValue.type, markup: marker, place: formValue.place, details: formValue.details, description: formValue.description}).then(handleClose)
    }

    useEffect(() => {
        const abortController = new AbortController()
        const signal = abortController.signal
        db.toolOnAir({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, collection: "lostNfound", signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
                            ...
                            <Table striped bordered hover size="sm" className="text-center">
                                ...
                                <tbody>
                                    {info.map(flow =>(
                                        ...
                                        <td>{flow.details}</td>
                                        <td>{flow.author}</td>
                                        <td className="bg-dark"><Button variant="outline-danger" size="sm" onClick={()=>db.deleteDocument({hotelId: userDB.hotelId, region: userDB.hotelRegion, departement: userDB.hotelDept, collection: "lostNfound", document: flow.id})}>Supprimer</Button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </Table>
