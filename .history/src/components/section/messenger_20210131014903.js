    ...
    const handleSubmit = (event) =>{
        ...
        uploadTask.on(
          ...
          () => {
              ...
              .then(url => {
                const uploadTask = () => {
                    ...
                    if(date !== today) {
                        ...
                        firebase.addNote({documentId: user.displayName, noteId: title, author: user.username, text: note, img: url, status: status, markup: marker, userId: user.uid, date: date})
            ...
            if(date !== today) {
                const notif = "Votre message a bien été enregistré pour le " + date + " ."
                firebase.addNote({documentId: user.displayName, noteId: title, author: user.username, text: note, img: url, status: status, hour:  markup: marker, userId: user.uid, date: date})
                firebase.addNotification({documentId: user.displayName, notification: notif})
                setStartDate(new Date)
                return setShowModal(false)
            }else{
    ...
    return(
        <div className="messenger_container">
            ...
            <PerfectScrollbar>
                ...
                <img src={Plus} alt="Plus" style={{width: "1%", cursor: "pointer", marginRight: "1vw", position: "fixed", right: "45vw", filter: "drop-shadow(2px 2px 2px)"}} onClick={handleShow} />
            </OverlayTrigger>
            </PerfectScrollbar>
