...
export default function DeepMap2({user, firebase}) {
    ...
    const [initialFilter, setInitialFilter] = useState("classement")
    const [departement, setDepartement] = useState("YVELINES")
    const [filter, setFilter] = useState(["Toutes les étoiles", "O étoiles"])
    const [operator, setOperator] = useState("array-contains-any")
    ...
    useEffect(() => {
        ...
        firebase.hotelDataOnAir({region: region, departement: departement, initialFilter: initialFilter, filter: filter, operator: operator, signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
     ...
     },[region, departement, filter, initialFilter, operator])
