...
import 'moment/locale/fr'
  ...
  useEffect(() => {
    ...
    firebase.iziUserOnAir2({userId: user.email, signal : signal}).onSnapshot(function(snapshot) {
                const snapInfo = []
  ...
  return (
    <div style={{
        ...
        {info.map(doc => (
            <span>
                <img src={Tips} style={{width: "3vw", marginRight: "1vw", cursor: "pointer"}} onClick={()=>{
                    let tipsAmount = doc.tips
                    let tipsUp = tipsAmount + 1
                    let userAccount = doc.id
                    return firebase.updateTips({userId: userAccount, tips: tipsUp})
                }} />
                <>{doc.tips} tips</>
            </span>))}
