...
export default function DarkProfilDetails({firebase, user}) {
    ...
    useEffect(() => {
        ...
        firebase.iziUserOnAir({documentId: user.displayName, collection: "cab", signal : signal}).onSnapshot(function(snapshot) {
                    const snapInfo = []
    ...
    return (
        <div style={{
            ...
            <Avatar 
                ...
                style={{filter: "drop-shadow(2px 2px 2px)"}}    />
            <h2 style={{filter: "drop-shadow(2px 2px 2px)", marginTop: "1vh"}}>{user.username}</h2>
