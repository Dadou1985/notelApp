...
export default function DeepMap2({user, firebase}) {
    ...
    const [hotelRef, sethotelRef] = useState(initialState)
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        ...
        if(info !== []){
            firebase.hotelRatingDataOnAir({region: region, departement: departement, hotelId: info, signal : signal}).onSnapshot(function(snapshot) {
                const snapInfo = []
              snapshot.forEach(function(doc) {          
                snapInfo.push({
                    id: doc.id,
