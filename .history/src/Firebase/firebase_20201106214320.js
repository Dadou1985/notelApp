...
class Firebase {
  ...
  async userIsConnected ({userEmail}){
    return this.db.collection("iziUsers")
    .doc(userEmail)
    .update({
      isConnected: true
  ...
  async userIsDisconnected ({userEmail}){
    return this.db.collection("iziUsers")
    .doc(userEmail)
    .update({
      isConnected: false
  ...
  async login({email, password}) {
    await this.auth.signInWithEmailAndPassword(email, password).then(()=>navigate('/yinYanPage'));
    return this.userIsConnected({email})
  }
