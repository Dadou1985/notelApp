...
class Firebase {
  ...
  async getUserProfile({userId, refHotel}){
    return this.db.collection('hotels').where('userId', '==', userId).get();
  }
  ...
  async adminWorkspaceRegister({email, password, username, userId, refSpace}) {
  ...
  async workspaceRegister({email, password, username, userId, refSpace}) {
    await this.auth.currentUser.updateProfile({displayName: refSpace})
    return this.db.collection("hotels")
    .doc(`${refSpace}`)
    .collection("users")
  ...
  async login({email, password}) {
    return this.auth.signInWithEmailAndPassword(email, password).then(()=>navigate('/doorsStage'));
  }
